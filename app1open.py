...
from src.openai_client import OpenAIAnalyzer  # CHANGED: From gemini_client to openai_client
...
def main():
    ...
    if 'raw_data' not in st.session_state:
        ...
        # CHANGED: From GeminiAnalyzer to OpenAIAnalyzer
        st.session_state.openai_analyzer = OpenAIAnalyzer()
...
def show_categorization(categorized_folder: str):
    ...
    st.markdown("""
        Categorize incidents using a pipeline: Keyword Matching → ML (SentenceTransformer + XGBoost) → OpenAI GPT-4 Fallback.
    ...
    if df_hash == st.session_state.last_df_hash and os.path.exists(categorized_file_path):
        try:
            with st.spinner("Loading previously categorized data for this file..."):
                ...
                col3.metric("OpenAI GPT-4 Fallback Cases", len(categorized_df[categorized_df['Classification Method'] == 'LLM']))
                ...
                if not confidence_data.empty:
                    ...
                    st.info("No confidence scores available (all cases classified by OpenAI GPT-4).")
    ...
    if st.button("🚀 Run Categorization", type="primary"):
        ...
        llm_progress = st.progress(0, text="Step 3: OpenAI GPT-4 Fallback...")
        ...
        try:
            ...
            if llm_cases > 0:
                with st.spinner(f"Using OpenAI GPT-4 fallback for {llm_cases} cases..."):
                    llm_progress.progress(50, text=f"Step 3: OpenAI GPT-4 Fallback ({llm_cases} cases, 50% complete)")
                    log_container.info(f"Completed OpenAI GPT-4 fallback for **{llm_cases} cases**.")
                    llm_progress.progress(100, text="Step 3: OpenAI GPT-4 Fallback (Complete)")
            else:
                llm_progress.progress(100, text="Step 3: OpenAI GPT-4 Fallback (Skipped)")
                log_container.info("No cases required OpenAI GPT-4 fallback.")
            ...
            col1.metric("Total Incidents", len(categorized_df))
            col2.metric("Avg. Confidence (Keyword/ML)", f"{confidence_avg:.2%}" if not pd.isna(confidence_avg) else "N/A")
            col3.metric("OpenAI GPT-4 Fallback Cases", llm_cases)
...
def show_dashboard(df: pd.DataFrame, selected_domain: str):
    ...
    st.subheader("♻️ Reopen Insights - AI Powered")
    openai_insights_key = f'openai_reopen_insights_{selected_domain}'
    if openai_insights_key not in st.session_state:
        st.session_state[openai_insights_key] = None
    df_reopened_subset = st.session_state.dashboard.get_reopened_incidents_subset(df)
    if not df_reopened_subset.empty:
        if st.button("Generate AI Insights", key=f"ai_insights_button_{selected_domain}"):
            with st.spinner("Analyzing with OpenAI GPT-4..."):
                ...
                st.session_state[openai_insights_key] = st.session_state.dashboard.get_openai_reopen_insights(prompt_text)
        if st.session_state[openai_insights_key] is not None:
            st.write(st.session_state[openai_insights_key])
        else:
...
def show_settings():
    ...
    with col1:
        ...
        openai_api_key = st.text_input("Azure OpenAI API Key", type="password", help="Enter your Azure OpenAI API key")
        openai_endpoint = st.text_input("Azure OpenAI Endpoint", value="https://openai-dev-aiops.openai.azure.com/")
        openai_deployment = st.text_input("Azure OpenAI Deployment Name", value="AIOPS_DEV_OAI_gpt-4o")
